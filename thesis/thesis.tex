...
Despite the recognized importance of addressing flaky tests, detecting them remains a formidable challenge. 
...
A flaky failure refers to a test failure that is not due to a bug in the \ac{sut}, but rather occurs by chance. 
...
To evaluate our methodology, we selected two open source projects as case studies: 
...
There are several approaches to flakiness detection.
In this section, we want to give an overview of some published approaches.
...
Others suggest using up to five reruns \autocite{lam_understanding_2020} or as many as 10,000 \autocite{alshammari_flakeflagger_2021} to detect flakiness.
...
For the second case study, \textsc{n8n}, the average runtimes are higher when the instrumentation is used.
