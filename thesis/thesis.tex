...
\feedback{Research Questions: evaluation seems late in the thesis to pose your research questsions}
...
Code coverage is often used to identify parts of the code not executed by the test suite \autocite{marick_brian_and_smith_john_and_jones_mark_how_1999}.
These parts of the code can be a good start for the development of new tests to ensure the test suite covers the entire system.
It is also sometimes misused as a measure of the quality of a test suite \autocite{fowler_bliki_2012}.
However, \citeauthor*{fowler_bliki_2012} also suggests that the quality of a test suite can be measured by the number of bugs that make it into production and willingness of developers changing code that could lead to bugs \autocite{fowler_bliki_2012}.
...
Then, we review existing work on flaky failure detection and the effect of instrumentation on flakiness.
...
\Citeauthor*{lam_idflakies_2019} introduce a framework called \textsc{iDFlakies} to detect flaky tests.
...
\Citeauthor*{alshammari_flakeflagger_2021} present a tool called \textsc{FlakeFlagger} that predicts whether tests are likely to be flaky without re-running them.
...
\Citeauthor*{haben_importance_2023} studied the difference between detecting flaky tests and flaky test failures, highlighting the importance of the latter for a more effective \ac{ci} process.
...
To address this problem, the authors propose a novel technique called \textsc{DeFlaker}.
...
The effectiveness was evaluated on 26 Java projects hosted on \textit{TravisCI}, where it identified 91 previously unknown flaky failures.
...
Given the importance of detecting flaky failures rather than flaky tests \autocite{haben_importance_2023}, the approach presented by \citeauthor*{bell_deflaker_2018} shows promising results for Java unit tests \autocite{bell_deflaker_2018}.
...
Developers could benefit substantially from immediate and reliable feedback on any failures that occur.
...
If there is no interception of the code that was covered by the failing test execution and the code that changed, the failure is assumed to be flaky \autocite{bell_deflaker_2018}.
...
The node.js server uses the V8 coverage \ac{api} \autocite{basques_coverage_2020} to collect coverage data.
...
The XML report is transformed using the \textit{jacoco-parse} library \autocite{noauthor_jacoco-parse_2023}\footnote{We maintain a fork of the library with security updates at \url{https://github.com/heddendorp/jacoco-parse} which is used in this project.}.
...
To collect coverage data we use the V8 native coverage (covered in the next section).
...
Instead of following the approach suggested in the cypress docs \autocite{noauthor_code_nodate}, we used the native V8 coverage \ac{api} to collect coverage data.
...
First, we describe the setup use to evaluate our approach.
...
The \textsc{ArTEMiS} \ac{ci} pipeline is hosted on \textit{Bamboo} \autocite{atlassian_bamboo_nodate} and consists of multiple plans.
...
Even though we filtered out apparent cases of issues with the \ac{ci} pipeline, the \ac{ci} pipeline was still showing some unstable behavior.
We kept the builds that were affected by these issues in our evaluation as they still showed realistic behavior of the \ac{ci} pipeline.
...
While our workflow still relies on the original repository, we had to make some changes to the \ac{e2e} tests to make them work with our instrumentation.
...
Lastly, we calculated the precision, recall, and f1 score for the approach in general by combining the results from both study subjects weighted by number of failures that were analyzed.
...
For the second study subject, \textsc{n8n}, the mean runtimes are higher when the instrumentation is used.
...
% In cases where the tests fail, the groups have fewer than 30 samples, we will still use a t-test because it has been shown to be robust to violations of the normality assumption.
Event with a group smaller than 30 we still use a t-test as it has been shown to be robust to violations of the normality assumption \autocite{blanca_non-normal_2017}.
...
The chi-squared test shows that the more frequently reported failures do not show a significant difference between the instrumented and not instrumented runs ($\chi^2(1) = 2.41, p = .121$).
...
While the results in \cref{results_rq1} show that our approach identifies flaky errors with a good precision of \num{0.73} but a low recall of \num{0.42}.
While we believe that false positives are a much bigger problem than false negatives, missing more half of the flaky failures does not go far in solving the problems caused by flaky failures.
...
We suspect that the low recall is due to the fact that \ac{e2e} tests cover considerably more code than unit tests.
...
And while our approach finds less than half of the flaky failures, it also comes at a significant cost in terms of runtime and implementation effort.
...
\begin{mdframed}
	...
	While other studies suggest that instrumentation does not have a significant effect on the failure rate of \ac{e2e} tests \autocite{rasheed_effect_2023}, we cannot draw a definitive conclusion.
\end{mdframed}
%------- chapter 6 -------
...
In this thesis, we have conceptualized and evaluated an approach for detecting flaky failures in \ac{e2e} tests using change coverage.
...
\Citeauthor*{bell_deflaker_2018} reports a recall of \num{0.955} and a precision of \num{0.985}. \cite{bell_deflaker_2018}.
