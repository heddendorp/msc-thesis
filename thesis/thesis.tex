...
\begin{enumerate}
	\item[\textbf{RQ\textsubscript{1}:}] How effective is the adapted Deflaker approach in identifying flaky failures in UI end-to-end tests?
	\item[\textbf{RQ\textsubscript{2}:}] What impact does the instrumentation have on the behaviour of the application and tests?
		\begin{enumerate}
			\item[\textbf{RQ\textsubscript{2.1}:}] To what extent does instrumentation impact the temporal behavior of the application during end-to-end testing?
			\item[\textbf{RQ\textsubscript{2.2}:}] How does the instrumentation of coverage collection affect the failure rate of end-to-end tests in the studied projects?
		\end{enumerate}
\end{enumerate}
